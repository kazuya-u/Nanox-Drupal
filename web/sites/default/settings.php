...
if (file_exists($app_root . '/' . $site_path . '/settings.local.php')) {
  ...
  if (file_exists(DRUPAL_ROOT . '/../services/redis/redis.inc')) {
    require DRUPAL_ROOT . '/../services/redis/redis.inc';
  }
  if (file_exists(DRUPAL_ROOT . '/../services/memcached/memcache.inc')) {
    require DRUPAL_ROOT . '/../services/memcached/memcache.inc';
  }
}
elseif (!empty($_ENV['AH_SITE_GROUP'])) {
  if (file_exists('/var/www/site-php')) {
    require '/var/www/site-php/' . $_ENV['AH_SITE_GROUP'] . '/' . $_ENV['AH_SITE_GROUP'] . '-settings.inc';
    if (file_exists(DRUPAL_ROOT . '/../vendor/acquia/memcache-settings/memcache.settings.php')) {
  ...
  switch (DRUPAL_ROOT) {
    case '__DEV_DOCROOT_PATH__':
      ...
      $settings['memcache']['key_prefix'] = 'dev_drupal_sandbox';
      $settings['varnish_purge_zeroconfig_hosts'] = ['dev.drupal.sandbox'];
      $settings['varnish_purge_zeroconfig_port'] = 80;
      $settings['trusted_host_patterns'] = ['^dev\.drupal\.sandbox$'];
      $config['system.logging']['error_level'] = 'verbose';
      break;

    case '__STG_DOCROOT_PATH__':
      $databases['default']['default'] = [
        'database' => '________',
        'username' => '________',
        'password' => '________',
      ...
      $settings['memcache']['servers'] = ['localhost:11211' => 'default'];
      $settings['memcache']['key_prefix'] = 'stg_drupal_sandbox';
      $settings['varnish_purge_zeroconfig_hosts'] = ['stg.drupal.sandbox'];
      $settings['varnish_purge_zeroconfig_port'] = 80;
      $settings['trusted_host_patterns'] = ['^stg\.drupal\.sandbox$'];
      $config['system.logging']['error_level'] = 'verbose';
      break;

    case '__PROD_DOCROOT_PATH__':
      $databases['default']['default'] = [
        'database' => '________',
        'username' => '________',
        'password' => '________',
      ...
      $settings['memcache']['servers'] = ['localhost:11211' => 'default'];
      $settings['memcache']['key_prefix'] = 'www_drupal_sandbox';
      $settings['varnish_purge_zeroconfig_hosts'] = ['www.drupal.sandbox'];
      $settings['varnish_purge_zeroconfig_port'] = 80;
      $settings['trusted_host_patterns'] = ['^(www\.)?drupal\.sandbox$'];
      break;
  }
...
// Prevents running out of memory when running Drush.
if (PHP_SAPI === 'cli') {
